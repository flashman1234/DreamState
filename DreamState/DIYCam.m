...
- (void)setup
{
    if ([self isVideoCameraAvailable])
    {
        ...
        // Inputs
        // ---------------------------------
        AVCaptureDevice *videoDevice    = [self cameraWithPosition:DEVICE_PRIMARY];
 ...
 * @return  void
 */
- (void)startVideoCapture:(NSString*)fileName
{    
    if (session != nil)
    {
        ...
        NSString *assetPath         = [self createAssetFilePath:@"mov" fileName:fileName];
...
- (NSString *)createAssetFilePath:(NSString *)extension fileName:(NSString *)fileName
{
